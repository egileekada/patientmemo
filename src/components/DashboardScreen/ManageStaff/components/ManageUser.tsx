...
export default function ManageUser(props: any) { 
    ...
    const { isLoading, data, refetch } = useQuery('user', () =>
        fetch(`${API_BASE_ENDPOINT}/admins`, {
            method: 'GET', // or 'PUT'
            headers: {
    ...
    const DeleteHandler =async(index: any)=> {
        await fetch(`${API_BASE_ENDPOINT}/admins/remove-staff/${index}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                Authorization : `Bearer ${localStorage.getItem('token')}`
    ...
    const ClickHandler =(index: any, name: any,)=>{
        setDeleteModal(true)
        setIndex(index)
        setPatient(name)
    }
                                        ...
                                        return(
                                            <Tr className={'font-Ubuntu-Medium cursor-pointer text-black text-sm'} key={index} >
                                                ...
                                                <Td className=' relative z-30  ' >
                                                    <svg onClick={()=> ClickHandler(item._id,item?.firstName+" "+item?.lastName)} className="cursor-pointer relative z-30 " width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <>
                                    <div className="h-auto flex justify-center items-center overflow-x-hidden overflow-y-hidden fixed inset-0 z-50 outline-none focus:outline-none"> 
                                        <div className='w-80 rounded-lg flex flex-col justify-center items-center bg-white p-8' >
                                            ...
                                            <p className=' font-Inter-Medium text-sm mt-3 text-black text-center' >Do You Want To Delete {patient} Account ?</p>
