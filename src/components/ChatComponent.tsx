...
export default function ChatComponent() { 
    ...
    return (
        <div className=' w-[350px] border-l h-full overflow-y-auto pr-4 py-10 rounded-xl border-[#C8C8C8] flex px-4 flex-col  ' >
            ...
            {!isLoading && (
                <> 
                    {data.filter((item:any) => item._id !== userData._id).map((item: any, index: any) => {
                        if(index < 7){ 
                            return(  
                                <button onClick={()=> navigate("/admin-chat")} key={index} style={{boxShadow: "0px 4px 10px 0px #6C6C6C26"}} className=' my-3 p-4 rounded-md flex items-center ' >
